...
router.put("/profile", isLoggedIn, (req, res, next) => {
  ...
  User.findByIdAndUpdate(req.user._id, updates).then(user => {
    res.json({
      success: true
    });
  });
...
router.post("/picture", parser.single("picture"), (req, res, next) => {
  let id = req.user._id;
  cloudinary.v2.uploader.destroy(id, function(res) {
    console.log(res);
  });
  User.findByIdAndUpdate(id, { imageURL: req.file.url }).then(() => {
    res.json({
      success: true,
